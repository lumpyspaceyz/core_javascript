...
const shopOrder = {
  ...
  getTotalPrice() {
    ...
    // // arrow function을 안 쓰면(= 일반 함수면) this는 window
    // this.menu.forEach((item) => {
    //   this.total += item.price * item.count
    // })

    // Array.forEach + 일반 함수(concise function)
    // // forEach 반복문에서 일반 함수로 this 사용하는 법
    // // forEach 두 번째 인자인 this로 일반 함수의 this를 대체하는 행위
...
const totalPrice = shopOrder.getTotalPrice();
...
// Array.forEach 반복문 -> 배열을 반복할 때는 배열의 메소드 만큼 좋은 게 없음
